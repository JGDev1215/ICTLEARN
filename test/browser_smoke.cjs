...
test('static page preserves SVG rollback and renders the default interactive replay without external requests', {timeout: 30000}, async t => {
  ...
  await page.waitForFunction(() => window.ictChartApp?.getAudit().dataMode === 'static' && window.ictChartApp.getAudit().totalBars === 180);
  ...
  await page.waitForFunction(() => window.ictChartApp.getAudit().timeframe === 5 && window.ictChartApp.getAudit().totalBars === 36);
  await page.selectOption('#chart-timeframe', '1');
  await page.waitForFunction(() => window.ictChartApp.getAudit().timeframe === 1 && window.ictChartApp.getAudit().totalBars === 180);
  await page.selectOption('#trading-session', 'london');
  await page.waitForFunction(() => window.ictChartApp.getAudit().session === 'london');
  await page.selectOption('#trading-session', 'nyam');
  await page.waitForFunction(() => window.ictChartApp.getAudit().session === 'nyam');
  await page.fill('#trade-date', '2026-08-31');
  await page.locator('#trade-date').dispatchEvent('change');
  await page.waitForFunction(() => window.ictChartApp.getAudit().date === '2026-08-31');
  ...
  await page.click('#compare-day');
  await page.fill('#compare-date', '2026-09-03');
  await page.selectOption('#compare-session', 'nyam');
  await page.click('#compare-form button[type="submit"]');
  await page.waitForFunction(() => Boolean(window.ictChartApp.getAudit().scene.comparison));
  assert.equal((await page.evaluate(() => window.ictChartApp.getAudit().scene.comparison.date)), '2026-09-03');
